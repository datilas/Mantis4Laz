...
type
  ...
  published
    ...
    property OnThreadSynchronize: TThreadSynchronizeEvent Read FOnThreadSynchronize Write FOnThreadSynchronize;
  end;
...
resourcestring
  ...
  // Form Titles
  SExceptionInfo = 'Informação da exceção';
  SCallStack = '&Trilha da exceção';
  SGeneral = '&Geral';
  SErrorOccured = 'Ocorreu um erro durante a execução do programa:';
  STerminate = '&Sair do programa';
  SClose = '&Continuar';
  SDetails = '&Detalhes';
  SIgnoreNextTime = 'Ignorar esta exceção da proxima vez';
  ...
  SUnit = 'Unit';
  SExceptionHandlerCannotBeSynchronized = 'O manipulador de exceção não pode ser sincronizado com o thread principal.';

  SErrorReportFileNotCrearted = 'Arquivo de relatório não criado';
...
begin
  SubAddLine(SReportTime, FormatDateTime('dd/mm/yyyy hh:nn:ss.zzz', Now));
  // OS Info
...
begin
  if not Assigned(FLastException) then Exit;
...
begin
  if FIgnoreList.IndexOf(FLastException.ClassName) <> -1 then Exit;
  ...
  try
    ...
    begin
      try
        ...
        on E: Exception do
          FLoggerLastError := SErrorReportFileNotCrearted + '. ' + E.Message;
      end;
    end;
  finally
...
function TExceptionLogger.FormatBasicDataReport(ABasicData: TStringList): TStringList;
var
  fieldName, fieldValue: string;
  i: integer;
begin
  Result := TStringList.Create;
  if not Assigned(ABasicData) then Exit;
  if ABasicData.Count = 0 then Exit;
...
procedure TExceptionLogger.ThreadSynchronize(AObject: TObject; Method: TThreadMethod);
begin
  if AObject is TThread then
    TThread.Synchronize(TThread(AObject), Method)
  else
    raise Exception.Create(SExceptionHandlerCannotBeSynchronized);
end;
...
begin
  ...
  Result := Format('%ddays %dhours %dmin %dsec %dms', [days, hours, minutes, seconds, ms]);
end;
